...
import { subscribe, closeSocket } from "@/lib/websocket"
                ...
                try {
                    ...
                    try {
                        ...
                        const timeoutPromise = new Promise((_, reject) => 
                            setTimeout(() => reject(new Error('Jobs API timeout')), 3000)
                        )
                        
                        const jobsList = await Promise.race([jobsPromise, timeoutPromise])
                                     .catch(error => {
                                        console.warn(`Could not fetch jobs for ${set}:`, error)
                                        return []
                                     })
...
useEffect(() => {
    ...
    const handleJobStatusChange = async (data: {
        ...
        if (data.status === "completed" || data.status === "failed" || data.status === "aborted") {
            debouncedRefresh()
        } else if (data.status === "processing" || data.status === "paused" || data.status === "pending") {
            if (vectorSetInfo[data.vectorSetName]) {
                setVectorSetInfo((prev) => ({
                    ...prev,
                    [data.vectorSetName]: {
                        ...prev[data.vectorSetName],
                        activeJobs: (prev[data.vectorSetName]?.activeJobs || 0) + 1,
                    },
                }))
            }
        }
