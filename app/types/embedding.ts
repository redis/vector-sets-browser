...
export function validateAndCorrectMetadata(data: unknown): VectorSetMetadata {
    ...
    if (typedData.embedding && typeof typedData.embedding === 'object') {
        ...
        if ('provider' in embeddingData && 
            typeof embeddingData.provider === 'string' &&
            ['openai', 'ollama', 'tensorflow', 'image', 'none'].includes(embeddingData.provider)) {
            correctedMetadata.embedding.provider = embeddingData.provider as EmbeddingProvider;
            
            // Copy the provider-specific config if it exists
            switch (embeddingData.provider) {
                case 'openai':
                    if (embeddingData.openai && typeof embeddingData.openai === 'object') {
                        correctedMetadata.embedding.openai = {
                            apiKey: embeddingData.openai.apiKey || '',
                            model: embeddingData.openai.model || 'text-embedding-3-small'
                        };
                        if (embeddingData.openai.batchSize) {
                            correctedMetadata.embedding.openai.batchSize = embeddingData.openai.batchSize;
                        }
                    }
                    break;
                case 'ollama':
                    if (embeddingData.ollama && typeof embeddingData.ollama === 'object') {
                        const ollamaData = embeddingData.ollama as Record<string, unknown>;
                        correctedMetadata.embedding.ollama = {
                            apiUrl: typeof ollamaData.apiUrl === 'string' ? ollamaData.apiUrl : 'http://localhost:11434',
                            modelName: typeof ollamaData.modelName === 'string' ? ollamaData.modelName : 'all-minilm'
                        };
                        if (typeof ollamaData.promptTemplate === 'string') {
                            correctedMetadata.embedding.ollama.promptTemplate = ollamaData.promptTemplate;
                        }
                    }
                    break;
                case 'tensorflow':
                    if (embeddingData.tensorflow && typeof embeddingData.tensorflow === 'object') {
                        const tensorflowData = embeddingData.tensorflow as Record<string, unknown>;
                        correctedMetadata.embedding.tensorflow = {
                            model: typeof tensorflowData.model === 'string' ? 
                                tensorflowData.model as TensorFlowModelName : 'universal-sentence-encoder'
                        };
                    }
                    break;
                case 'image':
                    if (embeddingData.image && typeof embeddingData.image === 'object') {
                        const imageData = embeddingData.image as Record<string, unknown>;
                        correctedMetadata.embedding.image = {
                            model: typeof imageData.model === 'string' ? 
                                imageData.model as ImageModelName : 'mobilenet'
                        };
                        if (typeof imageData.inputSize === 'number') {
                            correctedMetadata.embedding.image.inputSize = imageData.inputSize;
                        }
                    }
                    break;
                case 'none':
                    if (embeddingData.none && typeof embeddingData.none === 'object') {
                        const noneData = embeddingData.none as Record<string, unknown>;
                        correctedMetadata.embedding.none = {
                            model: typeof noneData.model === 'string' ? noneData.model : 'unknown',
                            dimensions: typeof noneData.dimensions === 'number' ? noneData.dimensions : 0
                        };
                    }
                    break;
            }
    ...
    correctedMetadata.dimensions = modelData?.dimensions || 
                                  (typeof typedData.dimensions === 'number' ? 
                                   typedData.dimensions : 0);
    ...
    if (typedData.redisConfig && typeof typedData.redisConfig === 'object') {
        ...
        if (typeof redisConfigData.reduceDimensions === 'number') {
            correctedMetadata.redisConfig.reduceDimensions = redisConfigData.reduceDimensions;
        }
        
        if (typeof redisConfigData.defaultCAS === 'boolean') {
            correctedMetadata.redisConfig.defaultCAS = redisConfigData.defaultCAS;
        }
        
        if (typeof redisConfigData.quantization === 'string' && 
            ['Q8', 'BIN', 'NOQUANT'].includes(redisConfigData.quantization)) {
            correctedMetadata.redisConfig.quantization = redisConfigData.quantization as 'Q8' | 'BIN' | 'NOQUANT';
        }
        
        if (typeof redisConfigData.buildExplorationFactor === 'number') {
            correctedMetadata.redisConfig.buildExplorationFactor = redisConfigData.buildExplorationFactor;
        }
    }
