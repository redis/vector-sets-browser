...
import { ForceNode, ForceEdge, LayoutAlgorithm, LayoutAlgorithmType } from "../types"
...
export function useLayoutManager(
    ...
    const normalizeAndScaleProjection = useCallback((
        ...
        const nodeCountFactor = Math.log(projection.length + 1) / Math.log(20)
    ...
    const umapLayout: LayoutAlgorithm = {
        ...
        apply: async (nodes) => {
            ...
            try {
                ...
                const normalizedEmbedding = normalizeAndScaleProjection(embedding)
            ...
            try {
                ...
                nodes.forEach((node) => {
                    if (node.vector) {
                        ...
                        // use 0,0,0
                        console.log("ERROR: [useLayoutManager] No vector for node:", node)
                        //vectors.push([0, 0, 0])
                        //nodeOrder.push(node)
                    }
                })
                ...
                // Normalize and scale the projection
                const normalizedEmbedding = normalizeAndScaleProjection(pcaEmbedding)
