...
export function useVisualizationState() {
    ...
    const loadColorScheme = useCallback(async () => {
        try {
            const storedScheme = await userSettings.get("colorScheme")
    ...
    const saveColorScheme = useCallback(async () => {
        try {
            await userSettings.set("colorScheme", isDarkMode ? "dark" : "light")
        } catch (error) {
    ...
    const loadLineVisibility = useCallback(async () => {
        try {
            const storedVisibility = await userSettings.get("showLines")
    ...
    const saveLineVisibility = useCallback(async () => {
        try {
            await userSettings.set("showLines", showLines)
        } catch (error) {
    ...
    const loadCardPinState = useCallback(async () => {
        try {
            const storedPinState = await userSettings.get("isCardPinned")
    ...
    const saveCardPinState = useCallback(async () => {
        try {
            await userSettings.set("isCardPinned", isCardPinned)
        } catch (error) {
