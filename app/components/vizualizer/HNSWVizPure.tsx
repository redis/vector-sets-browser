...
const HNSWVizPure: React.FC<HNSWVizPureProps> = ({
    ...
    useEffect(() => {
        const initializeVisualization = async () => {
            ...
            console.log("[HNSWVizPure] Creating initial mesh with element: ", initialElement)
            const initialMesh = createNodeMesh(initialElement.element, initialElement.vector)
    ...
    const expandNode = useCallback(
        async (node: THREE.Mesh, skipLayoutReapplication?: boolean) => {
            ...
            if (!node.userData.expanded) {
                ...
                const neighborsToProcess = response.result.slice(0, remainingSlots)
                
                for (const item of neighborsToProcess) {
                    // Check if this element already exists in any node
                    const elementAlreadyExists = nodesRef.current.some(
                        existingNode => existingNode.mesh.userData.element === item.element
                    );
    ...
    const handleCopyVector = async () => {
        ...
        try {
            if (selectedNode.userData.vector) {
                const vectorString = JSON.stringify(selectedNode.userData.vector)
