...
import { jobs, type Job, type ImportLogEntry } from "@/app/api/jobs"
...
export default function ImportTab({ vectorSetName, metadata }: ImportTabProps) {
    ...
    const fetchJobs = useCallback(async () => {
        ...
        try {
            ...
            if (data) {
                const filteredJobs = data.filter(job => !dismissedJobIds.has(job.jobId))
                ...
                filteredJobs.forEach(job => {
                    if (job.status.status === "paused") {
                        ...
                        const pausedTime = job.status.message?.match(/paused at (\d+)/)?.[1]
                        if (pausedTime) {
                            const pausedTimestamp = parseInt(pausedTime, 10)
                            const elapsedMinutes = (now - pausedTimestamp) / (1000 * 60)
                            
                            // If paused for more than 5 minutes, mark it for cleanup
                            if (elapsedMinutes > 5) {
                                console.log(`Job ${job.jobId} has been paused for ${elapsedMinutes.toFixed(1)} minutes, marking for cleanup`)
                ...
                setJobList(filteredJobs.filter(job => !dismissedJobIds.has(job.jobId)))
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (Math.random() < 0.1) { // ~10% chance each poll, so roughly every 20 seconds
                fetchImportLogs()
            }
        }, 2000)
                                            ...
                                            >
                                                ...
                                                <AlertDescription className="flex justify-between items-center">
                                                    ...
                                                    <Button 
                                                        variant="outline"
                                                        size="sm"
                                                        onClick={() => forceCleanupJob(job.jobId)}
                                                    >
                                                        Dismiss
                                                    </Button>
                                                </AlertDescription>
