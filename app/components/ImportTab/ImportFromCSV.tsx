...
import { type EmbeddingTemplateResponse } from "@/app/api/openai"
...
export default function ImportFromCSV({
    ...
    const [selectedElementColumn, setSelectedElementColumn] = useState<string>("")
    ...
    const [suggestedElementTemplate, setSuggestedElementTemplate] = useState<string>("")
    const [suggestedVectorTemplate, setSuggestedVectorTemplate] = useState<string>("")
    ...
    const [isOpenAIKeyAvailable, setIsOpenAIKeyAvailable] = useState<boolean | null>(null)
    ...
    useEffect(() => {
        const checkOpenAIKey = () => {
            ...
            try {
                ...
                // For client-side, we'll just check if localStorage has a key
    ...
    const saveOpenAIKey = () => {
        if (!openAIKey.trim()) {
            setOpenAIKeyError("Please enter a valid OpenAI API key");
            return;
        }
        ...
        try {
            ...
            // If we already have CSV data, generate templates
            if (csvPreview?.headers && csvPreview.sampleRows.length > 0) {
                generateTemplateSuggestions(csvPreview.headers, csvPreview.sampleRows);
            }
        } catch (error) {
            console.error('Error saving OpenAI key:', error);
            setOpenAIKeyError("An error occurred while saving the API key");
        }
    };

    // Function to generate template suggestions using OpenAI
    const generateTemplateSuggestions = async (headers: string[], sampleRows: Record<string, string>[]) => {
        ...
        try {
            ...
            const sampleRowsArray = sampleRows.map(row => Object.values(row).join(", "));

            const template = await generateEmbeddingTemplate(headers, sampleRowsArray)
            ...
            setError('Failed to generate template suggestions. You can still create templates manually.');
            // If we get an authentication error, the key might be invalid
            if (error.toString().includes('authentication') || error.toString().includes('API key')) {
                setIsOpenAIKeyAvailable(false);
            }
        } finally {
    ...
    const insertColumnIntoTemplate = (template: string, column: string, setter: (value: string) => void, textareaRef: React.RefObject<HTMLTextAreaElement>) => {
        if (textareaRef.current) {
            const textarea = textareaRef.current;
            const start = textarea.selectionStart;
            const end = textarea.selectionEnd;
            
            const newTemplate = template.substring(0, start) + 
                `\${${column}}` + 
    ...
    const renderHighlightedTemplate = (template: string) => {
        ...
        while ((match = regex.exec(template)) !== null) {
            ...
            if (match.index > lastIndex) {
                parts.push(
                    <span key={key++}>{template.substring(lastIndex, match.index)}</span>
                );
            }
        ...
        if (lastIndex < template.length) {
            parts.push(
                <span key={key++}>{template.substring(lastIndex)}</span>
            );
        }
    ...
    const handleImport = async () => {
        ...
        if (activeTab === "manual") {
            if (!selectedElementColumn) {
                setError("Please select a column to use as the element identifier")
                return
            }
        ...
        const config: ImportJobConfig = {
            ...
            hasHeader: true,
            skipRows: 0,
            elementColumn: activeTab === "manual" ? selectedElementColumn : csvPreview.headers[0],
            textColumn: activeTab === "manual" ? selectedVectorColumn : csvPreview.headers[0],
            elementTemplate: activeTab === "ai" ? elementTemplate : undefined,
            textTemplate: activeTab === "ai" ? vectorTemplate : undefined,
            attributeColumns: selectedAttributeColumns.length > 0 ? selectedAttributeColumns : undefined,
    ...
    const renderTemplateWithData = (template: string, rowIndex: number) => {
        ...
        csvPreview.headers.forEach(header => {
            ...
            result = result.replace(regex, row[header] || '');
        });
        ...
        return (
            <div className="space-y-6 py-4">
                <Alert>
                    ...
                    <AlertDescription>
                        The AI Import feature requires an OpenAI API key to analyze your CSV data and create optimal templates.
                    </AlertDescription>
                </Alert>
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Your API key will be stored securely in your browser. You can get an API key from{" "}
                            <a 
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                ...
                                <TabsContent value="manual" className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <Label htmlFor="element-column">Element Identifier Column</Label>
                                            <Select 
                                                value={selectedElementColumn} 
                                                onValueChange={setSelectedElementColumn}
                                            >
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                This column will be used as the unique identifier for each vector
                                            </p>
                                        </div>

                                        <div className="space-y-2">
                                            <Label htmlFor="vector-column">Vector Content Column</Label>
                                            <Select 
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                This column&apos;s text will be encoded as a vector
                                            </p>
                                        </div>
                                    </div>
                                </TabsContent>
                                    ...
                                    {isCheckingOpenAIKey ? (
                                        ...
                                        <>
                                            <Alert className="mb-4">
                                                ...
                                                <AlertDescription>
                                                    AI Import automatically creates optimal templates for your data by analyzing your CSV columns and content.
                                                </AlertDescription>
                                            </Alert>
                                            ...
                                            {!isGeneratingTemplates && suggestedElementTemplate && suggestedVectorTemplate && (
                                                <div className="space-y-6">
                                                    <div className="space-y-3 border rounded-lg p-4">
                                                        <div className="flex items-center justify-between">
                                                            <Label className="text-base font-medium">AI Suggested Element Name</Label>
                                                            <Button 
                                                                variant="ghost" 
                                                                size="sm" 
                                                                onClick={() => setShowElementEditor(!showElementEditor)}
                                                            >
                                                                {showElementEditor ? "Hide Editor" : "Edit"}
                                                            </Button>
                                                        </div>
                                                        
                                                        <div className="p-3 bg-muted rounded-md">
                                                            {renderHighlightedTemplate(elementTemplate)}
                                                        </div>
                                                        
                                                        {showElementEditor && (
                                                            <div className="space-y-2 mt-3 pt-3 border-t">
                                                                <Label htmlFor="element-template">Edit Element Template</Label>
                                                                <Textarea 
                                                                    id="element-template" 
                                                                    ref={elementTemplateRef}
                                                                    value={elementTemplate}
                                                                    onChange={(e) => setElementTemplate(e.target.value)}
                                                                ...
                                                                <div className="flex flex-wrap gap-1 mt-2">
                                                                    {csvPreview.headers.map(header => (
                                                                        <Button 
                                                                            ...
                                                                            variant="outline" 
                                                                            size="sm"
                                                                            onClick={() => insertColumnIntoTemplate(elementTemplate, header, setElementTemplate, elementTemplateRef)}
                                                                        >
                                                                            {header}
                                                                        </Button>
                                                                    ))}
                                                    ...
                                                    <div className="space-y-3 border rounded-lg p-4">
                                                        <div className="flex items-center justify-between">
                                                            <Label className="text-base font-medium">AI Suggested Embedding Template</Label>
                                                            <Button 
                                                                variant="ghost" 
                                                                size="sm" 
                                                                onClick={() => setShowVectorEditor(!showVectorEditor)}
                                                            >
                                                                {showVectorEditor ? "Hide Editor" : "Edit"}
                                                            </Button>
                                                        </div>
                                                        
                                                        <div className="p-3 bg-muted rounded-md">
                                                            {renderHighlightedTemplate(vectorTemplate)}
                                                        </div>
                                                        
                                                        {showVectorEditor && (
                                                            <div className="space-y-2 mt-3 pt-3 border-t">
                                                                <Label htmlFor="vector-template">Edit Embedding Template</Label>
                                                                <Textarea 
                                                                    id="vector-template" 
                                                                    ref={vectorTemplateRef}
                                                                    value={vectorTemplate}
                                                                    onChange={(e) => setVectorTemplate(e.target.value)}
                                                                ...
                                                                <div className="flex flex-wrap gap-1 mt-2">
                                                                    {csvPreview.headers.map(header => (
                                                                        <Button 
                                                                            ...
                                                                            variant="outline" 
                                                                            size="sm"
                                                                            onClick={() => insertColumnIntoTemplate(vectorTemplate, header, setVectorTemplate, vectorTemplateRef)}
                                                                        >
                                                                            {header}
                                                                        </Button>
                                                                    ))}
                                                                ...
                                                                <p className="text-xs text-muted-foreground mt-2">
                                                                    For best results, include all relevant information in the template. The more context provided, the better the embedding will represent your data.
                                                                </p>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="mt-4">
                                                        ...
                                                        {showPreview && csvPreview.sampleRows.length > 0 && (
                                                                ...
                                                                <div className="space-y-3">
                                                                    <div>
                                                                        ...
                                                                        <div className="p-2 bg-muted rounded-md text-sm mt-1">
                                                                            {renderTemplateWithData(elementTemplate, previewSampleIndex)}
                                                                        </div>
                                                                    </div>
                                                                    
                                                                    <div>
                                                                        <Label className="text-xs">Embedding Text:</Label>
                                                                        <div className="p-2 bg-muted rounded-md text-sm mt-1 max-h-[150px] overflow-y-auto">
                                                                            {renderTemplateWithData(vectorTemplate, previewSampleIndex)}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                    ...
                                    <div className="space-y-2">
                                        {csvPreview.headers.map(header => (
                                            <div key={header} className="flex items-center space-x-2">
                                                <Checkbox 
                                                    id={`attr-${header}`} 
                                                    checked={selectedAttributeColumns.includes(header)}
                                                    onCheckedChange={() => handleAttributeToggle(header)}
                                                />
                                                <Label htmlFor={`attr-${header}`}>{header}</Label>
                                            </div>
                                        ))}
                                    </div>
                                </ScrollArea>
                                <p className="text-xs text-muted-foreground">
                                    Selected columns will be stored as attributes with each vector
                                </p>
                            </div>
                    ...
                    {csvPreview && (
                        <Button
                            ...
                            disabled={isImporting || !metadata?.embedding || 
                                (activeTab === "manual" && (!selectedElementColumn || !selectedVectorColumn)) ||
                                (activeTab === "ai" && (!elementTemplate || !vectorTemplate))}
                        >
