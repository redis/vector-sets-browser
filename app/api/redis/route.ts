...
async function handleRedisOperation<T>(
    ...
    try {
        ...
        if (
            ...
            error.message.includes("No Redis connection available")
        ) {
            return NextResponse.json(
                { error: "No Redis connection available" },
                { status: 401 }
    ...
    try {
        ...
        switch (action) {
            ...
            case "deleteVectorSet":
                return handleRedisOperation((url) =>
                    redis.deleteVectorSet(url, params.keyName)
                )
            ...
            case "vdim":
                return handleRedisOperation((url) =>
                    redis.vdim(url, params.keyName)
                )

            case "vcard":
                return handleRedisOperation((url) =>
                    redis.vcard(url, params.keyName)
                )

            case "vrem":
                return handleRedisOperation((url) =>
                    redis.vrem(url, params.keyName, params.element)
                )

            case "vemb":
                return handleRedisOperation((url) =>
                    redis.vemb(url, params.keyName, params.element)
                )
            case "vlink":
                return handleRedisOperation((url) =>
                    redis.vlink(url, params.keyName, params.element)
                )
            ...
            case "getMetadata":
                return handleRedisOperation((url) =>
                    redis.getMetadata(url, params.keyName)
                )

            case "setMetadata":
                return handleRedisOperation((url) =>
                    redis.setMetadata(url, params.keyName, params.metadata)
                )

            case "memoryUsage":
                return handleRedisOperation((url) =>
                    redis.getMemoryUsage(url, params.keyName)
                )
