...
async function getCacheConfig() {
    try {
        ...
        return await RedisClient.withConnection(url, async (client) => {
            ...
            if (!result.success) {
                console.error(
                    "[Cache] Error getting cache config:",
                    result.error
                )
                return DEFAULT_CACHE_CONFIG
            }
...
async function getCacheMetadata(): Promise<CacheMetadata> {
    try {
        ...
        return await RedisClient.withConnection(url, async (client) => {
            ...
            if (!result.success) {
                console.error(
                    "[Cache] Error getting cache metadata:",
                    result.error
                )
                return { entries: {} }
            }
...
async function updateCacheMetadata(metadata: CacheMetadata): Promise<void> {
    try {
        ...
        await RedisClient.withConnection(url, async (client) => {
            ...
            if (!result.success) {
                console.error(
                    "[Cache] Error updating cache metadata:",
                    result.error
                )
            }
        })
...
async function getTensorFlowEmbedding(
    ...
    try {
        ...
        console.log(
            `[Embedding] TensorFlow.js returned embedding of length ${embedding.length}`
        )
...
async function getImageModelEmbedding(
    ...
    try {
        ...
        if (embedding.length !== expectedDim) {
            console.error(
                `[Embedding] Unexpected image embedding dimension: got ${embedding.length}, expected ${expectedDim}`
            )
            throw new Error(
        ...
        console.log(
            `[Embedding] Image model returned embedding of length ${embedding.length}`
        )
        console.log("[Embedding] Sample values:", embedding.slice(0, 5))
...
async function getEmbedding(
    ...
    if (config.provider === "ollama" && config.ollama) {
        ...
        if (cached) {
            console.log(
                "[Embedding] Cache hit - returning cached embedding for Ollama"
            )
            return cached
        }
        ...
        if (!response.ok) {
            ...
            console.error(
                `[Embedding] Ollama API error: ${response.status} - ${errorText}`
            )
            throw new Error(
        ...
        if (!validationResult.isValid) {
            throw new Error(
                `Invalid Ollama embedding: ${validationResult.error}`
            )
        }
        ...
        console.log(
            `[Embedding] Ollama returned embedding of length ${embedding.length}`
        )
        console.log("[Embedding] Sample values:", embedding.slice(0, 5))
        ...
        console.log(
            `[Embedding] Calling OpenAI API with model ${config.openai.model}`
        )
        ...
        if (!response.ok) {
            const errorText = await response.text()
            console.error(
                `[Embedding] OpenAI API error: ${response.status} - ${errorText}`
            )
            throw new Error(
        ...
        if (embedding.length !== expectedDim) {
            console.error(
                `[Embedding] Unexpected embedding dimension: got ${embedding.length}, expected ${expectedDim}`
            )
            throw new Error(
        ...
        console.log(
            `[Embedding] OpenAI returned embedding of length ${embedding.length}`
        )
        return embedding
        ...
        throw new Error(
            "Unsupported embedding provider or invalid configuration"
        )
    }
}
