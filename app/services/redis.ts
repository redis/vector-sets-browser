...
export async function vadd(keyName: string, element: string, vector: number[]): Promise<number> {
  const response = await callRedisApi("vadd", { keyName, element, vector });
  if (!response?.success) {
...
export async function vemb(keyName: string, element: string): Promise<number[]> {
  if (!keyName) {
  ...
  try {
    ...
    if (!response?.success) {
      throw new Error(`VEMB operation failed: ${response?.error || 'Unknown error'}`);
    }
...
export async function getVectorWithSimilarity(keyName: string, id: string, similarity: number): Promise<[string, number, number[]]> {
  try {
