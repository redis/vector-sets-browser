  ...
  try {
    ...
    eventSource.onerror = (error) => {
      ...
      if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
        reconnectAttempts++;
        setTimeout(() => {
          console.log(`Attempting to reconnect SSE (${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})...`);
          initSocket();
        }, RECONNECT_DELAY * reconnectAttempts); // Exponential backoff
      } else {
        console.warn('Max SSE reconnection attempts reached, using polling fallback.');
      }
    };
  } catch (error) {
 ...
 */
export function subscribe(eventType: AppEvents, handler: (payload: any) => void): () => void {
    ...
    fetch('/api/jobs')
      ...
      .then(data => {
        if (data.success) {
          ...
          const jobsByVectorSet: Record<string, Record<string, number>> = {};
          ...
          Object.keys(jobsByVectorSet).forEach(vectorSetName => {
            Object.keys(jobsByVectorSet[vectorSetName]).forEach(status => {
              if (status === 'processing' || status === 'paused' || status === 'pending') {
                eventBus.emit(AppEvents.JOB_STATUS_CHANGED, {
                  vectorSetName,
                  status,
